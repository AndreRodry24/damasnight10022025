...
async function tratarAdvertencia(c, groupId, userId) {
    ...
    if (advertencias[userId] >= 3) {
        ...
        await sendMessage(c, groupId, `@${userId.split('@')[0]}, *foi removido do grupo* 👏🍻 *DﾑMﾑS* 💃🔥 *Dﾑ* *NIGӇԵ* 💃🎶🍾🍸 *devido a três advertências anteriores.* \n\n*Por favor, revisem as regras para evitar futuras penalizações.*`, userId);
        delete advertencias[userId]; // Reiniciar o contador
    } else {
        await sendMessage(c, groupId, `@${userId.split('@')[0]}, você recebeu uma *ADVERTÊNCIA* ${advertencias[userId]}/3 ⚠️ por violar as regras do grupo 👏🍻 *DﾑMﾑS* 💃🔥 *Dﾑ* *NIGӇԵ* 💃🎶🍾🍸 \nPor favor, leia as regras antes de postar. *Lembre-se:* ao atingir *três advertências*, você será removido do grupo.❌`, userId);
    }
}
